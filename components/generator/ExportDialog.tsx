...
export const ExportDialog = memo(function ExportDialog({ open, onOpenChange, theme }: ExportDialogProps) {
  const [format, setFormat] = useState<'css' | 'scss' | 'sass' | 'json'>('css')
  const [code, setCode] = useState('')
  const { generateCss, generateScss, generateSass, generateJson } = useWasm()
  ...
  const generateCode = useCallback(() => {
    ...
    setCode(generated)
  }, [format, theme, generateCss, generateScss, generateSass, generateJson])
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl h-[90vh] p-0 gap-0 overflow-hidden flex flex-col">
        {/* Enhanced Header */}
        <div className="flex-shrink-0 bg-gradient-to-br from-primary/10 via-background to-secondary/10 p-6 border-b border-border/50">
          <DialogHeader className="space-y-3">
            <DialogTitle className="text-3xl font-bold font-heading bg-gradient-to-r from-foreground via-foreground/90 to-foreground/70 bg-clip-text text-transparent">
              Export Your Theme
            </DialogTitle>
        ...
        <div className="flex-1 min-h-0 overflow-hidden flex flex-col p-6">
          <Tabs value={format} onValueChange={(v) => setFormat(v as typeof format)} className="flex flex-col h-full min-h-0">
            {/* Improved Tabs */}
            <TabsList className="flex-shrink-0 grid w-full grid-cols-4 bg-muted/30 backdrop-blur-sm h-10 mb-4 rounded-md border border-border/50">
              <TabsTrigger 
                value="css" 
                className="text-xs font-medium font-mono data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-sm transition-all rounded-md"
              >
                CSS
              </TabsTrigger>
              <TabsTrigger 
            ...
            <TabsContent value={format} className="flex-1 min-h-0 flex flex-col mt-0 overflow-hidden">
              <div className="flex flex-col h-full min-h-0 space-y-3">
                ...
                <div className="flex-shrink-0 flex gap-2">
                  <Button 
                    ...
                    variant="outline" 
                    size="sm"
                    className="h-9 px-4 text-xs shadow-sm hover:shadow transition-shadow"
                  >
                  ...
                  <Button 
                    ...
                    size="sm"
                    className="h-9 px-4 text-xs shadow-sm hover:shadow-md transition-shadow bg-gradient-to-r from-primary to-primary/90"
                  >
                ...
                {/* Footer Info Bar */}
                <div className="flex-shrink-0 flex items-center justify-between text-xs text-muted-foreground bg-muted/40 backdrop-blur-sm p-3 rounded-md border border-border/50">
              ...
              <Textarea
                value={code}
                readOnly
                    className="font-mono text-xs w-full resize-none border-0 rounded-none focus-visible:ring-0 bg-transparent p-5 leading-relaxed"
                    style={{ minHeight: '100%' }}
              />
                </div>
            </div>
